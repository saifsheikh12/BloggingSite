...
const authorisationMidd = async function (req, res, next) {
    try {
        ...
        if (req.params.blogId) { let blog = await blogModel.findById(req.params.blogId).select({ authorId: 1 });
         if (blog.authorId != token.authorid) return res.status(401).send({ status: false, msg: "UnAuthorised" }); next() };
    }
    catch (error) {
        return res.status(500).send({ status: false, msg: error.message });
    }
